...
class coco_karpathy_train(Dataset):
    def __init__(self, transform, image_root, ann_root, max_words=30, prompt='', client=None):        
        ...
        if self.client is not None:
            ...
            self.annotation = json.loads(client.get(os.path.join('s3://sdcBucket/BLIP-main', ann_root, filename)))
        else:
...
class coco_karpathy_caption_eval(Dataset):
    def __init__(self, transform, image_root, ann_root, split, client=None):  
        '''
        image_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
