...
from timm.optim.optim_factory import (Optional, torch, optim, AdaBelief, Nadam, RAdam, AdamP, SGDP,
    Adafactor, Adahessian, RMSpropTF, NovoGrad, NvNovoGrad, Lookahead, nn)
...
def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
    ...
    for name, param in model.named_parameters():
        if (not param.requires_grad) or ('alpha' in name): # for UPop
...
def optimizer_kwargs(cfg, search):
...
def create_optimizer(args, model, filter_bias_and_bn=True, search=False):
    """ Legacy optimizer factory for backwards compatibility.
    NOTE: Use create_optimizer_v2 for new code.
    """
    return create_optimizer_v2(
        model,
        **optimizer_kwargs(cfg=args, search=search),
